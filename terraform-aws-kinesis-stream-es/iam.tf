resource "aws_iam_role" "firehose_es_stream_role" {
...
resource "aws_iam_role_policy" "firehose_es_stream_policy" {
  count = var.elasticsearch_enabled ? 1 : 0

  role = aws_iam_role.firehose_es_stream_role[0].name
  ...
  policy = jsonencode({
    ...
    "Statement" : [
      ...
      },
      {
        ...
        "Resource" : "*"
 #        [
 #         "arn:aws:firehose:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:deliverystream/${var.kinesis_firehose_name}",
 #         "arn:aws:firehose:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:stream/${var.aws_kinesis_stream_name}"
 #         ]
      },
      {
        "Effect" : "Allow",
